...
namespace NokLib.Pooling
{
    ...
    public class ConcurrentObjectPool<T> : ObjectPoolBase<T> where T : class
    {
        ...
        public ConcurrentObjectPool(Func<T> createObject = null, Action<T> onGet = null, Action<T> onRelease = null, Action<T> onDispose = null, int capacity = DEFAULT_MAX_CAPACITY, int initialAllocation = DEFAULT_INIT_ALLOCATION, bool collectionCheck = DEFAULT_COLLECTION_CHECK) : base(createObject, onGet, onRelease, onDispose, capacity, initialAllocation, collectionCheck) { }
